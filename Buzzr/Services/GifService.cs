...
public class GifService
{
    ...
    public static async Task<List<GifResult>> SearchAsync(string query, int limit = 24)
    {
        ...
        {
            var url = $"{baseUrl}/{key}/gifs/search?q={Uri.EscapeDataString(query)}&per_page={limit}&customer_id=buzzr&content_filter=medium";
    ...
    public static async Task<List<GifResult>> GetTrendingAsync(int limit = 24)
    {
        ...
        foreach (var baseUrl in BaseUrls)
        {
            var url = $"{baseUrl}/{key}/gifs/trending?per_page={limit}&customer_id=buzzr";
            var results = await FetchAsync(url);
            if (results.Count > 0) return results;
            if (LastError == null) return [];
        }
